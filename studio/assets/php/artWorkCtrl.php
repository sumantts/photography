...
if ($result->num_rows > 0) {
    while($row = $result->fetch_array()){
        ...
        $post_aWork = new stdClass();			
        $post_aWork->post_id = $post_id;			
        $post_aWork->category_name = $category_name;
        $post_aWork->author_name = $author_name;
        $post_aWork->post_title = $post_title;
        $post_aWork->post_description = $post_description1;
        $post_aWork->post_image = $post_image;
        $post_aWork->post_video = $post_video;
        $post_aWork->post_tags = $post_tags;
        $post_aWork->created_on = $created_on;

        array_push($posts_aWork, $post_aWork);
    }
}
//$mysqli->close();
?>